...
export default function LeftNav() {
  ...
  const sortedCountries = organizedLeagues
    .sort((a, b) => {
      if (sortOrder === "asc") {
  ...
  const handleToggleCountriesView = () => {
    setCountriesView((prevView) =>
      prevView === 31 ? organizedLeagues.length : 31
    );
  };

  return (
    <div className="bg-projectGrays-500 min-w-[260px] p-4 pb-1 rounded-lg h-full m-2 flex flex-col">
        <div className="flex w-full justify-between mb-1">
          <h1 className="font-bold">Ligas</h1>
          <div className="flex w-fullitems-center justify-center">
            <button
              className="transition-all duration-100 hover:bg-projectGrays-500 p-1 rounded-md"
              onClick={() => {
                showItems("");
                setCountriesView(31);
              }}
        ...
        <div className="w-full flex justify-center items-center mt-2">
          <ToggleCountriesButton
            data={countriesView}
            toggleFN={handleToggleCountriesView}
          />
        </div>
    </div>
  );
